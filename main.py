'''
처음 인풋값이 폴더라면 폴더안의 내용을 감지해서 각자 객체로 만든다
처음 인풋값이 파일이라면 그 파일이 어떤 파일인지 인식해서 객체로 만든다

객체의 개수는 여러개일 수 있으므로 한 목록에서 꺼내면서 작업한다
...
while True:
    ...
    while True:
        try:
            lists = filter_csv_from_dir(input("> ")) #TODO: input 수정하기
            break
